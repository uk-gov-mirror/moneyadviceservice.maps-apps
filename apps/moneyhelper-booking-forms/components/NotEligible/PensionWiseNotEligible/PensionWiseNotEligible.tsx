...
export const PensionWiseNotEligible: BookingStepComponent = () => {
  const { tList } = useTranslation();
  const componentKey = `components.${StepName.PENSION_WISE_NOT_ELIGIBLE}`;
    ...
    <div className="flex flex-col gap-4">
      <Callout className="mt-4">
        <SectionsRenderer
          ...
          testIdPrefix={`${StepName.PENSION_WISE_NOT_ELIGIBLE}-callout`}
        />
      </Callout>
      <SectionsRenderer
        sections={sections}
        testIdPrefix={StepName.PENSION_WISE_NOT_ELIGIBLE}
      />
    </div>
  );
};
