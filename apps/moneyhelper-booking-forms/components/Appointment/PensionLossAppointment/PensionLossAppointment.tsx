...
export const PensionLossAppointment: BookingStepComponent = ({ step }) => {
  ...
  const componentKey = `components.${StepName.PENSION_LOSS_APPOINTMENT}`;
    ...
    <>
      <div className="flex flex-col gap-4">
        ...
        <SectionsRenderer
          ...
          testIdPrefix={StepName.PENSION_LOSS_APPOINTMENT}
        />
        <H2 className="text-blue-700">{t(`${componentKey}.callout.title`)}</H2>
        <Callout>
          <SectionsRenderer
            sections={calloutSections}
            testIdPrefix={`${StepName.PENSION_LOSS_APPOINTMENT}-callout`}
          />
        </Callout>
      </div>
      <FormWrapper
