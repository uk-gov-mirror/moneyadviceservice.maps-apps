...
export const SelfEmployedAppointment: BookingStepComponent = ({ step }) => {
  ...
  const componentKey = `components.${StepName.SELF_EMPLOYED_APPOINTMENT}`;
    ...
    <>
      <div className="flex flex-col gap-4">
        ...
        <SectionsRenderer
          ...
          testIdPrefix={StepName.SELF_EMPLOYED_APPOINTMENT}
        />
        <H2 className="text-blue-700">{t(`${componentKey}.callout.title`)}</H2>
        <Callout>
          <SectionsRenderer
            sections={calloutSections}
            testIdPrefix={`${StepName.SELF_EMPLOYED_APPOINTMENT}-callout`}
          />
        </Callout>
      </div>
      <FormWrapper
