...
describe('PensionCalculator component', () => {
  ...
  it('renders correctly step contributions part salary', () => {
    ...
    expect(
      screen.queryAllByText(
        /Employer contribution must be between 3 and 100%/i,
      ),
    ).toHaveLength(1);
    expect(
      screen.queryAllByText(/Total contribution must be at least 8%/i),
    ).toHaveLength(1);
  ...
  it('renders correctly step contributions full salary and shows max tax releif message', () => {
    ...
    (useRouter as jest.Mock).mockImplementation(() => ({
      push,
      route: 'workplace-pension-calculator',
      query: query,
      replace: jest.fn(),
      events: {
        on: jest.fn(),
    ...
    expect(push).toHaveBeenCalledWith(
      {
        hash: 'top',
        pathname: 'workplace-pension-calculator',
        query: {
          ...query,
