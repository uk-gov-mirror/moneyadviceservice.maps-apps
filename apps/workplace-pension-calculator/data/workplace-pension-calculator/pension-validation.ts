...
export const validateSalary = (
  ...
  if (Number.isNaN(Number(salaryFormatted))) {
    errors.salary = {
      field: 'salary',
      type: 'invalid',
    };
  ...
  if (
    ...
    !Number.isNaN(Number(salaryFormatted)) &&
...
export function validateContributions(
  ...
  const employeeInvalid = Number.isNaN(Number(employeeContribution));
  const employerInvalid = Number.isNaN(Number(employerContribution));
