...
test.describe('Workplace Pension Calculator', () => {
  ...
  /**
   * @tests 55900 - Should send results on an email
   */
  test('Check Email Contents', async ({
    ...
    await expect(yourResultsComponent.resultsMessage).toBeVisible();
    await expect(yourResultsComponent.resultsMessage).toContainText(
      data.messages.resultsMessage,
    );

    await expect(yourResultsComponent.emailResultsButton).toHaveAttribute(
      'href',
      `mailto:?body=${encodeURIComponent(data.messages.emailMessage)}`,
    );
  });
});
