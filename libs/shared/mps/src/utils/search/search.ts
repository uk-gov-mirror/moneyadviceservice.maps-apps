...
function compareDocumentTitles(
...
export function tokensCoveredBy(tokens: string[], texts: string[]): boolean {
  return tokens.some((token) => texts.some((text) => text.includes(token)));
}
...
export function documentMatchesKeywordWithParsed(
...
export function scoreSearchRelevanceWithParsed(
